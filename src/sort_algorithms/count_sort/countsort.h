...
#define COUNTSORT
#include<vector>
void count_sort(const std::vector<int>::iterator begin,const std::vector<int>::iterator end,const int max_val){
    std::vector<int> CountArray(max_val+1);
    ...
    std::vector<int> ResArray(std::distance(begin, end),0);
    for(auto it=end-1;it!=begin-1;it--){
        ResArray[CountArray[*it]-1]=*it;
        CountArray[*it]--;
    }
    std::copy(ResArray.begin(), ResArray.end(), begin);
